...
- **docs/index.html** (and docs/style.css if needed): Same intro text, same table (with sortable headers), same legend, same **Sources** section (same links and "As of" date), same **License & usage**. Table rows and `data-speed` / `data-intelligence` / `data-price` must match the README table.
...
- **Context**: e.g. `1M` or `128K` (from API/Vertex docs).
...
2. **Fill each column** from the right sources:
   - **Model, Context** → API docs or Vertex docs.
...
**Column → source:** Model, Context → API docs / Vertex. Speed, Purpose & use cases → Vertex Model Garden or blog. Intelligence (1–10) → ordinal from Artificial Analysis and model tier. Price → Google AI or Vertex pricing.
